    ...
    def _load_pdf_or_markdonw(self, file_path: str) -> List[Document]:
      ...
      try:
        ...
        if suffix == '.pdf':
            ...
            extract_tables = self.config.get('pdf', {}).get('extract_tables', False)
            use_ocr = self.config.get('pdf', {}).get('use_ocr', False)
            
            # Strategy 1: Enhanced extraction with tables
            if extract_tables:
                try:
                    from .enhanced_pdf_loader import EnhancedPDFLoader
                    self.logger.info(f"Using enhanced PDF loader for {file_path}")
                    loader = EnhancedPDFLoader(file_path)
                    return loader.load()
                except ImportError as e:
                    self.logger.warning(f"Enhanced PDF loader not available: {e}")
                except Exception as e:
            ...
            if use_ocr:
                try:
                    ...
                    self.logger.info(f"Using OCR PDF loader for {file_path}")
                    tesseract_path = self.config.get('pdf', {}).get('tesseract_path')
                    loader = OCRPDFLoader(file_path, use_ocr=True, tesseract_path=tesseract_path)
                    return loader.load()
                except ImportError as e:
                    self.logger.warning(f"OCR loader not available: {e}")
                except Exception as e:
                    self.logger.warning(f"OCR extraction failed: {e}")
            
            # Strategy 3: Default PyMuPDF (fast and reliable)
            self.logger.info(f"Using standard PyMuPDF loader for {file_path}")
            loader = PyMuPDFLoader(file_path)
            return loader.load()
        
        else:
            # For .md and other text formats
